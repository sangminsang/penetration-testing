                ...
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=30)) as response:
                    ...
                    if response.status == 200:
                        ...
                        if isinstance(data, list):
                            ...
                            # 단일 CVE 또는 에러 응답
                            if "id" in data:  # CVE 데이터
                                logger.info(f"[CVE-SEARCH] Found 1 CVE for {cpe}")
                                return [data]
                            else:
                                logger.warning(f"[CVE-SEARCH] Unexpected response format: {data}")
                                return []
                        else:
                            return []
                    
                    elif response.status == 404:
                        logger.info(f"[CVE-SEARCH] No CVEs found for {cpe} (404)")
                        return []
                    
                    else:
                        self.stats["failed_requests"] += 1
                        logger.warning(f"[CVE-SEARCH] HTTP {response.status} for CPE: {cpe}")
                        return []
